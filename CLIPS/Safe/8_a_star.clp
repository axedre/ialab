...
?f1 <- 	(apply (id ?curr) (op go-forward) (direction north) (pos-x ?r) (pos-y ?c))
    ...
    =>
        ...
        (assert	(newnode
            ...
            (direction north)
            (gcost (+ ?g 10))
            (fcost (+ (*(+(abs (- ?x (+ ?r 1))) (abs (- ?y ?c))) 10) ?g 10))
            (father ?curr))
        )
        (retract ?f1)
        (focus NEW)
...
?f1 <- 	(apply (id ?curr) (op go-right) (direction north) (pos-x ?r) (pos-y ?c))
        ...
        (assert	(newnode
            ...
            (direction east)
            (gcost (+ ?g 15))
            (fcost (+ (*(+(abs (- ?x ?r)) (abs (- ?y (+ ?c 1)))) 10) ?g 15))
            (father ?curr))
        )
        (retract ?f1)
        (focus NEW)
...
?f1 <- 	(apply (id ?curr) (op go-left) (direction north) (pos-x ?r) (pos-y ?c))
        ...
        (gcost (+ ?g 15)) (fcost (+ (*(+(abs (- ?x ?r)) (abs (- ?y (- ?c 1)))) 10) ?g 15))
                        (father ?curr)))
        (retract ?f1)
        (focus NEW)
)
...
?f1 <- 	(apply (id ?curr) (op go-forward) (direction west) (pos-x ?r) (pos-y ?c))
        ...
        (assert	(newnode
            ...
            (direction west)
            (gcost (+ ?g 10))
            (fcost (+ (*(+(abs (- ?x ?r)) (abs (- ?y (- ?c 1)))) 10) ?g 10))
            (father ?curr))
        )
        (retract ?f1)
        (focus NEW)
...
?f1 <- 	(apply (id ?curr) (op go-right) (direction west) (pos-x ?r) (pos-y ?c))
        ...
        (assert	(newnode
            ...
            (direction north)
            (gcost (+ ?g 15))
            (fcost (+ (*(+(abs (- ?x (+ ?r 1))) (abs (- ?y ?c))) 10) ?g 15))
            (father ?curr))
        )
        (retract ?f1)
        (focus NEW)
...
(defrule change-current
    ...
    ?f3 <- (lastnode (id ?last))
    =>
    (assert (current (id ?best)) (lastnode (id (+ ?last 13))))
    (retract ?f1 ?f3)
    (modify ?f2 (open no))
)
